...
export class HorariosService {
  ...
  getHorariosDisponiblesEspecialista(especialista_id: number, especialidad_id: number, empresa: number, cm: number, fecha: string) {
    return this.http.get(`${environment.apiUrl}/api/v1/horarios/horarios-disponibles`, {params: { 'especialista': especialista_id, 'especialidad': especialidad_id, 'empresa': empresa, 'cm': cm, 'fecha': fecha}})
                      .pipe(
                        map( (res: any) => {
                            return res;
                        }),

...
export class EstudiosGabineteComponent {
  ...
  getData(url?) {
    this.mantenimientoService.getDataMantenimiento('estudios-gabinete', this.empresaService.empresa_seleccionada.id).subscribe({
      next: (response: any) => {
  ...
  applyFilter(event: any) {
    ...
    this.mantenimientoService.getDataMantenimiento('estudios-gabinete', this.empresaService.empresa_seleccionada.id, filterValue).subscribe((response: any) => {
      this.registros = response.results;
    });
  }
}

...
export class AgregarClienteComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.registerForm.valid) {
        ...
        this.mantenimientoService.addObject('clientes', this.registerForm.value ).subscribe(
          (response) => {
            console.log(response);
              this.action.next( true );
              this.notificationService.showSuccess('Se creó el registro correctamente' , 'Cliente');
              this.modalRef.hide();
            },
            err => {
                  console.log(err);

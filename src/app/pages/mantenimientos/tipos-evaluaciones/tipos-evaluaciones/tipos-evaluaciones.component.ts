...
export class TiposEvaluacionesComponent {
  ...
  getData(url?) {
    this.mantenimientoService.getDataMantenimiento('tipos-evaluacion', this.sharedService.organizacion_seleccionada.id).subscribe({
      next: (response: any) => {
  ...
  applyFilter(event: any) {
    ...
    this.mantenimientoService.getDataMantenimiento('tipos-evaluacion', this.sharedService.organizacion_seleccionada.id, filterValue).subscribe((response: any) => {
      this.registros = response.results;
    });
  }
}

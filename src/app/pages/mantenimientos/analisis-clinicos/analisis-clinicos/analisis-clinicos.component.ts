...
export class AnalisisClinicosComponent {
  ...
  getData(url?) {
    this.mantenimientoService.getDataMantenimiento('analisis', this.empresaService.organizacion_seleccionada.id).subscribe({
      next: (response: any) => {
  ...
  applyFilter(event: any) {
    ...
    this.mantenimientoService.getDataMantenimiento('analisis', this.empresaService.organizacion_seleccionada.id, filterValue).subscribe((response: any) => {
      this.registros = response.results;
    });
  }
}

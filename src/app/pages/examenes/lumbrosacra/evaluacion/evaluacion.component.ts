...
export class EvaluacionComponent {
  ...
  options_aspecto = [
    {value: 1, label: 'Alineacion'},
    {value: 2, label: 'Exposicion'},
    {value: 3, label: 'Posición'},
    {value: 4, label: 'Proyección'},
  ]
        ...
        this.examenesService.updateEvaluacionFicha(this.sharedService.organizacion_seleccionada.id, this.sharedService.sucursal_seleccionada.id, this.programa, this.clave, this.slug, this.registro)
                            .subscribe({
                                next: (res: any) => {
                                  ...
                                  this.alertService.successSwalToast('Lumbrosacra Actualizada', 2000);
                                },
                                error: (err: any) => {
                                  console.log('error')
                                  this.disabled = false;
